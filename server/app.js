...
app.use("/", async (req,res,next) => {
    ...
    try {
        const decoded = jwt.verify(token, "secret-PortfolioUserToken-231sca");
        req.user = decoded;
        next();
    } catch (err) {
        if (err.name === "TokenExpiredError") {
            ...
            try {
                ...
                res.cookie("authToken", data.accessToken, {httpOnly: true});
                req.user = jwt.verify(data.accessToken, "secret-PortfolioUserToken-231sca");
                next()
            } catch (err) {
                if (isRedirected) {
                    next()
...
app.get("/profile/:profileNickname", async (req,res) => {
    ...
    try {
        ...
        jwt.verify(token, 'secret-PortfolioUserToken-231sca');
        notAuthorized = false;
    } catch (err) {
        notAuthorized = true;
    }
    ...
    try {
        token = req.cookies.authToken;
        currentUser = jwt.verify(token, 'secret-PortfolioUserToken-231sca');
    } catch (err) {
        currentUser = await User.findByNickname(req.params.profileNickname);
    }
...
app.get("/portfolio/:portfolioId", async (req,res) => {
    ...
    try {
        token = req.cookies.authToken;
        currentUser = jwt.verify(token, 'secret-PortfolioUserToken-231sca');
        currentUserData = await User.findByNickname(currentUser.nickname);
    } catch (err) {
        console.log(err)
        currentUser = await Profile.getByPortfolioId(req.params.portfolioId);
        let user = await User.findById(currentUser.user_id);
        currentUserData = await User.findByNickname(user.nickname) 
    ...
    if (!req.cookies.authToken) {
        return res.render("portfolio", {
            ...
            AvatarPath: userData.avatar_path || "icons/portfolio/user-icon.png",
            isGuest: true,
            notAuthorized: true
        })
    }
